...
import { type AdminLoginUseCase } from "../application/use-cases/admin-login.use-case";
...
export class AdminAuthController {
  ...
  login = async (
    ...
    try {
      const output = await this.adminLoginUseCase.execute(req.body);
      ...
      res.cookie(COOKIE_REFRESH_TOKEN, output.refreshToken, {
        ...
        secure: isProduction,
        sameSite,
        maxAge: (output.refreshTokenExpiresIn ?? DEFAULT_REFRESH_EXPIRY) * 1000,
        path: "/",
      });
